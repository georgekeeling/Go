...
namespace GoCarta
{
  public static class ModScore
  {
    ...
    public static Chain[] Chains = new Chain[101];
    public static Group[] Groups = new Group[71];
    ...
    private static int[,] StrongLinks = new int[101, 2];
    ...
    {
      // Verifies if the point (x, y) is friendly
    ...
    public static void StringAnalysis(int s, bool display, int killed = 0)
    {
      ...
      if (display)
      {
        // Reset image logic (if GUI is enabled)
      }
      ...
      for (i = 0; i <= N1; i++)
      {
        for (j = 0; j <= N1; j++)
        {
          ...
          if (Board[i, j] == 0 && Intensity[i, j] < 0 && !noTerritory.Contains(new Point(i, j)))
          {
            ...
            WhiteTerritory++;
#if GUI
                if (Shows) Main.Plot(Territory, i, j, Color.White);
#endif
          }
          if (Board[i, j] == 0 && Intensity[i, j] > 0 && !noTerritory.Contains(new Point(i, j)))
          {
            ...
            BlackTerritory++;
#if GUI
                if (Shows) Main.Plot(Territory, i, j, Color.Black);
#endif
          }
        }
      }
    ...
    public static int TerritoryCompute(int gro)
    {
      ...
      // Ignore friendly stones not belonging to the group
      for (int i = 0; i <= N1; i++)
      {
        for (int j = 0; j <= N1; j++)
        {
      ...
      // Mark points close to dame points
      for (int i = 0; i <= N1; i++)
      {
        for (int j = 0; j <= N1; j++)
        {
          ...
          {
            if (Internal(i + 1, j) && Board[i + 1, j] == 0 && Intensity[i + 1, j] == 0) noTerritory.Add(new Point(i, j));
            if (Internal(i - 1, j) && Board[i - 1, j] == 0 && Intensity[i - 1, j] == 0) noTerritory.Add(new Point(i, j));
            if (Internal(i, j + 1) && Board[i, j + 1] == 0 && Intensity[i, j + 1] == 0) noTerritory.Add(new Point(i, j));
            if (Internal(i, j - 1) && Board[i, j - 1] == 0 && Intensity[i, j - 1] == 0) noTerritory.Add(new Point(i, j));
          }
        }
      }
    ...
    private static void Erode()
    {
      ...
      int[,] bufferGoban = new int[19, 19];
      int[,] bufferIntensity = new int[19, 19];
    ...
    {
      // Returns one of the four contiguous points to point (i, j)
      return r switch
      {
        1 => new Point(i, j - 1),
        2 => new Point(i + 1, j),
    ...
    private static void ChainRemove(int s)
    {
      ...
      for (int k = 1; k <= Chains[s].Size; k++)
      {
        ...
        if (Shows)
        {
            Main.Plot(Removing, i, j, Color.Red);
        }
#endif
      }
    ...
    private static void AddNeighborPoints(int chainIndex, int cx, int cy)
    {
      // Adds neighbor points to the chain
      AddNeighborPoint(chainIndex, cx - 1, cy - 1);
      AddNeighborPoint(chainIndex, cx, cy - 1);
      AddNeighborPoint(chainIndex, cx + 1, cy - 1);
      AddNeighborPoint(chainIndex, cx - 1, cy);
      AddNeighborPoint(chainIndex, cx + 1, cy);
      AddNeighborPoint(chainIndex, cx - 1, cy + 1);
      AddNeighborPoint(chainIndex, cx, cy + 1);
      AddNeighborPoint(chainIndex, cx + 1, cy + 1);
    ...
    private static void AddNeighborPoint(int chainIndex, int x, int y)
    {
      if (Internal(x, y) && !Chains[chainIndex].NeighbourPoints.Contains(new Point(x, y)))
      {
        Chains[chainIndex].NeighbourPoints.Add(new Point(x, y));
        if (Board[x, y] == 0 && !Chains[chainIndex].EmptyNeighbourPoints.Contains(new Point(x, y)))
        {
          Chains[chainIndex].EmptyNeighbourPoints.Add(new Point(x, y));
        }
      }
    }
    ...
    public static double ScoreCompute(int NN1, bool japanese, double komi, int hc, int whitemoves, int blackmoves)
    {
      ...
      do
      {
        ...
        for (i = 1; i <= 100; i++) Chains[i] = default;
        for (i = 1; i <= 70; i++) Groups[i] = default;
        ...
        if (stat == 498)
        {
          if (!remov && !CheckSpecial) break;
          else CheckSpecial = false;
        }
      } while (true);
      ...
      // Restore the original position and repeat the process for white stones
      Array.Copy(OrigBoard, Board, Board.Length);
      Array.Copy(BackupIntensity, Intensity, Intensity.Length);

      // Fill dame points with white stones
      for (i = 0; i <= N1; i++)
      {
        for (j = 0; j <= N1; j++)
        {
      ...
      // Compare forced connections found by filling dame with black and white stones
      int minusBlack = 0;
      int minusWhite = 0;
      ...
      foreach (Point position2 in blackConnections)
      {
        if (whiteConnections.Contains(position2))
        {
          ...
          if (Math.Sign(Intensity[cx, cy]) > 0)
          {
            minusBlack++;
#if GUI
                if (Shows) Main.Plot(Connection, cx, cy, Color.Black);
#endif
          }
          else
          {
            minusWhite++;
#if GUI
                if (Shows) Main.Plot(Connection, cx, cy, Color.White);
#endif
          }
          CheckCaptures(cx, cy);
        }
      ...
      // Handle forced connections required to capture dead chains
      for (i = 0; i <= N1; i++)
      {
        for (j = 0; j <= N1; j++)
        {
          if (Intensity[i, j] == 0 && Board[i, j] != 0)
          {
            ...
            if (Funct(k, "LS") == 1)
            {
              ...
              if (!connections.Contains(atari))
              {
                ...
                {
                  colour[atari.X, atari.Y] = -Chains[k].Colour;
                  if (Chains[k].Colour == 1) minusBlack++;
                  else minusWhite++;
                }
      ...
      // Bring back to life stones in seki
      for (k = 1; k <= z; k++)
      {
    ...
    if (Shows)
    {
        Main.Score($"Black territory:      {BlackTerritory:##0}");
            ...
            if (Chains[j].Size > 0 && Chains[i].Colour == Chains[j].Colour && Funct(j, "LS") > 1)
            {
              ...
              for (k = 1; k <= Chains[i].Size; k++)
              {
                ...
                if (FC || (HC > 0 && counthalf[i] >= 1))
                {
                  ...
                  if (FC && (StrongLinks[Sl, 0] != i || StrongLinks[Sl, 1] != j))
                  {
                    Sl++;
                    StrongLinks[Sl, 0] = i;
                    StrongLinks[Sl, 1] = j;
      ...
      do
      {
        ...
        if (links[k, 0] != 0)
        {
          ...
          do
          {
            ...
            for (z = 1; z <= index; z++)
            {
              for (i = 1; i <= Groups[indexg].Size; i++)
              {
                if (links[z, 0] == Groups[indexg].Element[i])
                {
                  found2 = true;
                  for (j = 1; j <= Groups[indexg].Size; j++)
                  {
                ...
                if (links[z, 1] == Groups[indexg].Element[i])
                {
                  ...
                  if (found1)
                  {
                    Groups[indexg].Element[Groups[indexg].Size + 1] = links[z, 0];
                    Groups[indexg].Size++;
    ...
    private static bool FullConnection(int x, int y, int ch, bool checkLink)
    {
      ...
      // Point not empty = no extension, unless checkLink = true; in that case, the point may work as a link if the stone's color matches the chain's
      ...
      {
        // Diagonal jump: both side points must be empty/one of them is occupied by a friendly stone/opposite color cannot play there
        if (Internal(x + 1, y + 1) && ID[x + 1, y + 1] == ch)
        {
    ...
    private static void StatusCompute()
    {
      ...
      bool remove;
      Point pt1, pt2, pt3, eye, p, liberty;
          ...
          do
          {
            ...
            for (j = 0; j < Groups[i].Size; j++)
            {
              ...
              if (ch > 0 && Chains[ch].Size > 0)
              {
                ...
                Chains[ch].Status = (int)(520 - Math.Round(Chains[ch].EyesNumber / 2.0 + Chains[ch].Liberties * 2 + Chains[ch].GroupTerritory / 2.0));
              }
            }
      ...
      for (i = 0; i < 100; i++)
      {
        if (Chains[i].Size > 0)
        {
          ...
          for (l = 0; l < StrongLinks.Length; l++)
          {
    ...
    private static bool Killing(int ch)
    {
      ...
      if (Chains[ch].Size == 1 && Groups[Chains[ch].Group].Size == 1 && Chains[ch].Liberty.Count == 2)
      {
        good = true;
      ...
      if (Chains[ch].Size == 1 && Groups[Chains[ch].Group].Size == 1 && Chains[ch].Liberty.Count == 3)
      {
        lpot = 0;
      ...
      if (Chains[ch].Size == 1 && Groups[Chains[ch].Group].Size == 1 && Chains[ch].Liberty.Count == 4)
      {
        lpot = 0;
        foreach (var pt in Chains[ch].Liberty.Cast<Point>())
        {
      ...
      if (Chains[ch].Size == 2 && Groups[Chains[ch].Group].Size == 1 && Chains[ch].Liberty.Count == 2)
      {
        good = true;
        foreach (var pt in Chains[ch].Liberty.Cast<Point>())
        {
      ...
      if (Chains[ch].Size == 3 && Groups[Chains[ch].Group].Size == 1 && Chains[ch].Liberty.Count == 1)
      {
        good = true;
        foreach (var pt in Chains[ch].Liberty.Cast<Point>())
        {
      ...
      if (Chains[ch].Size == 4 && Groups[Chains[ch].Group].Size == 1 && Chains[ch].Liberty.Count == 2)
      {
        good = true;
        meanx = 0;
        meany = 0;
      ...
      if (good)
      {
        ...
        foreach (var pt in Chains[ch].Liberty.Cast<Point>())
        {
          // z = 0 means the "stealing eyes" chain is black, also worth an eye for white neighbor chains; z = 2 means the opposite.
    ...
    private static int SekiSearch()
    {
      ...
      for (i = 1; i <= 69; i++)
      {
        for (j = i + 1; j <= 70; j++)
        {
          if (Groups[i].Size > 0 && Groups[j].Size > 0 &&
              Chains[Groups[j].Element[1]].Colour != Chains[Groups[i].Element[1]].Colour &&
              Groups[i].Liberty.Count >= 2 && Groups[j].Liberty.Count >= 2 &&
              Groups[i].Territory - Groups[i].Eyes.Count == 0 &&
              Groups[j].Territory - Groups[j].Eyes.Count == 0 &&
              ...
              Chains[Groups[j].Element[1]].Status > 100)
          {
            ...
            if (same)
            {
              for (gro = 1; gro <= Groups[i].Size; gro++)
              {
              ...
              for (gro = 1; gro <= Groups[j].Size; gro++)
              {
    ...
    private static bool KillingShape(int ch)
    {
      ...
      if (Chains[ch].Size == 3 && Groups[Chains[ch].Group].Size == 1) good = true;

      // Four stones in a square are a "killing shape" too
      if (Chains[ch].Size == 4 && Groups[Chains[ch].Group].Size == 1)
      {
        good = true;
        meanX = 0;
        meanY = 0;
